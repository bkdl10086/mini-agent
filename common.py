...
def create_client(provider: str = "agent") -> OpenAI:
    """
    统一客户端工厂，靠 .env 命名前缀自动匹配。
    
    命名规则：{PROVIDER}_API_KEY / {PROVIDER}_BASE_URL / {PROVIDER}_MODEL
    
    provider:
      - "agent"  → 主推理模型（默认 deepseek）
      - "judge"  → 评估模型（默认百炼 qwen-max）
      - 任意自定义前缀，只要 .env 里有对应三项即可
    
    兼容：agent 模式下如果 AGENT_API_KEY 不存在，回退到 DEEPSEEK_API_KEY
    """
    load_dotenv(BASE_DIR / ".env")
...
def get_model(provider: str = "agent") -> str:
    ...
    load_dotenv(BASE_DIR / ".env")
    prefix = provider.upper()
    # agent 默认 deepseek-v4-flash，judge 默认 qwen-max
